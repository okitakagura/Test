...
bool HelloWorld::init()
{
	...
	//创建精灵
	m_pSprite=CCSprite::create("timg.jpg");
	setTouchEnabled(true);//开启响应触点功能
	m_pSprite->setPosition(ccp(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));

	this->addChild(m_pSprite, 1);//tag为成员标志
...
public:
	...
	virtual bool ccTouchBegan(CCSet *pTouches, CCEvent *pEvent)
	{
...
void HelloWorld::onTouchesMoved(__Set*pTouches, Event *pEvent)
{
	//获取触点指针容器中第一个元素
